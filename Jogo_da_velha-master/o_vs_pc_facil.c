...
//Este arquivo tem a principal função de ser o jogo do jogador como O contra CPU no facil, é a mesma coisa que a do X só mudando o layout

void o_vs_pc_rounds_facil(char velha[3][3], char amostrador[3][3], char nomeplayer[10], int qual_round){
    system("cls");
    ...
    printf("||                                                    ||\n");
        ...
        if(qual_round == 1 || qual_round == 3 || qual_round == 5 || qual_round == 7 || qual_round == 9){
            ...
            if(velha[linha][coluna] == 'X' || velha[linha][coluna] == 'O'){
                o_vs_pc_rounds_facil(velha, amostrador, nomeplayer, qual_round);}
