...
//Este arquivo tem a principal função de ser o jogo do jogador como X contra CPU no facil

void x_vs_pc_rounds_facil(char velha[3][3], char amostrador[3][3], char nomeplayer[10], int qual_round){ // Começa a função  do jogo do jogador como X contra a CPU
    ...
    printf("||                                                    ||\n");
        ...
        if(qual_round == 1 || qual_round == 3 || qual_round == 5 || qual_round == 7 || qual_round == 9){ //Caso seja vez do jogador pega sua escolha
            ...
            if(velha[linha][coluna] == 'X' || velha[linha][coluna] == 'O'){// Se não for válida ele chama a funçao round com o mesmo numéro de round
                printf("\n > > POSIÇÃO INVALIDA!! < <\n");
                system("pause");
                x_vs_pc_rounds_facil(velha, amostrador, nomeplayer, qual_round);}
          ...
          if(qual_round == 2 || qual_round == 4 || qual_round == 6 || qual_round == 8){//Vez da CPU
            ...
            //Pega um valor aleatorio entre 1 e 9 como se fosse uma escolha
            bole = 1;
            while(bole){
                aux = rand()%10;
            ...
            //Transforma essa escolha em coordenadas da matriz
                if (escolha == '1'){linha = 0; coluna = 0;}
                else if (escolha == '2'){linha = 0; coluna = 1;}
                else if(escolha == '3'){linha = 0; coluna = 2;}
                else  if(escolha == '4'){linha = 1; coluna = 0;}
            ...
            if(velha[linha][coluna] == 'X' || velha[linha][coluna] == 'O'){// Se não for válida ele chama a funçao round com o mesmo numéro de round
                ...
                amostrador[linha][coluna] = NULL;
                verificador_xvspc(velha, nomeplayer, qual_round); //verifica se alguem venceu
                qual_round++;
                x_vs_pc_rounds_facil(velha, amostrador, nomeplayer, qual_round);}
          }
